...
**多线程压缩流水线**：压缩全程改为"专职读线程 → N 计算工人 → 专职写线程"三段流水线（N = CPU 逻辑核数），缓冲池控内存上限。

实测性能（HuffmanAES，同机同输入对照，详见 `DevFiles/性能报告-2026-09-07.md`）：

| 场景 | 旧版 | 新版 | 提升 |
|---|---|---|---|
| 大文件集 4GiB（3,871 文件） | 214.3 s | 31.1 s | **6.9×** |
| 长路径 2.4GiB（33,835 文件，最长 267 字符） | 168.7 s | 47.2 s | **3.6×** |
| 极端小文件 15 万个（41 MiB） | 155.9 s | 86.1 s | **1.8×** |
