> **归档说明**：本文档描述的 DataBlocks、旧 ThreadPool 等模块已在重构中退役，内容仅作历史参考，不作为当前主要文档；现行设计见《开发流程与设计细节.md》与《线程池调研与改造计划.md》。（2026-09 归档）
...
**注意事项：**  
当前是数据块列表有两个及以上块时会触发多线程。更好的做法的可选择模式，存在单线程循环处理多块的解法。
