...
- **文件头**
  ...
  - 5. **文件头信息写入**
    ...
    - 1. 文件信息存储协议：|文件标|2B 文件名大小偏移量|文件名|----|
      ...
      - 为加密/分块特化的标签：文件标为字符 2：为分割偏移量（下一分区）写入|32bits 偏移量||16B iv 加密头|，如果读到偏移量为零，说明此时已经写入到最后一个不足 8KB 的块（按目录的残余偏移量来读取，包含只有一块的情况）aes 的加密头会写入到预留位置
...
- **文件头信息获取**
  ...
  4. 解压时数据获取：
     ...
     - 内存中保留拼接好的路径，抛弃已处理完的文件目录数据（动态数组采用默认 9KB 大小，实际上最多只能读到 8KB+600B 左右,配合 clear 恢复 size=0）对于超大文件名，目前暂时没有设计解决方案，初步设想：维护一个回退偏移量管理这种边界问题
  5. 压缩/解压时路径拼接细则：
     - 读到文件标0-目录进队等待处理（存完整路径），读到文件标1-文件，按当前路径拼接并且交予数据加载模块。
     - 遍历完第一层一个目录记录的“子文件数量”后。继续处理队列中的目录，此时可能是第一层目录，也可能开启了第二层循环。
     - 逻辑根目录需要在压缩时忽略
     - 路径（原始、压缩输出路径）：要在gui层新增传输给HeaderLoader模块的接口，不只是传给HeaderWriter。以fs::path传输
  6. 压缩实现细则：
     1. 跳过逻辑根，但是要获取数目
     2. 从gui获得一个路径vector。获取尾端目录/文件名-用于验证文件目录已经正确写入，验证通过直接使用路径
     3. 主循环：
        - I:读取目录块
        - II:读取到文件则拼接后读取数据块（windows不允许文件和文件夹同名，可以安全检索，但是linux允许）并且传给处理端（需要传输两次，第一次构建编码树，第二次真处理）
        - III:读取到目录则拼接后存其details到队列备用
        - IV:写入处理完的数据块，末尾写入一个编码表，按照分割标准分割，并且回填vector中的偏移量（偏移位置已保存）
        - V:按策略处理vector，而后
     4. 
