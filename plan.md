...
- **文件头**
  ...
  - 5. **文件头信息写入**
    ...
    - 1. 文件信息存储协议：|文件标|16bits 文件名大小偏移量|文件名|----|

      - 文件标为字符 0：为目录。需要写入：|32bits 第一级文件数量|
      - 文件标为字符 1：为文件。需要写入：|64bits 原文件大小|64bits 压缩后大小|。其中压缩大小暂时作为占位区。
      - 截断标为字符 2：为解压缩时分块加载目录留下分割标记，详见 2_3_VII。
    ...
    - 2. 压缩前运行：
      ...
      - 3. 搜索方案：采用 BFS。在上述 0、1、2 作为必要操作的条件下，搜索目录。
        ...
        VII:截断符，在完整写入文件目录信息的间隔中检测文件头此时是否达到8KB，如果是，那就写入截断字符 2，然后再继续写入。(由于顺序读取，结合存储协议的格式，这个2一定会被读到)
...
- **文件头信息获取**
  ...
  4. 解压时获取目录信息的分块策略：
     - 按分隔符形成的分块读取文件目录的已加密数据，解密后存储在内存中（需要专门编写一种新的读取函数，不过逻辑是类似的）
     - 内存中保留拼接好的路径，抛弃已处理完的文件目录数据（动态数组采用默认9KB大小，实际上最多只能读到8KB+600B左右,配合clear恢复size=0）
