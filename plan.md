...
- **关于文件头：**
  1. 记录文件头和数据块的偏移量：使用魔数或者其它更短的休止符号区分：
  - I:文件头做不到记录文件在整个压缩文件的绝对位置，所以需要记录从数据区开始的文件的大小。且文件头数据量小
  
  - II:文件头包含的信息是规定好的，不会出现预料外的数据，故用更短的休止符方便检测是更好的选择

  2. 记录数据块偏移量：具体来说，这些数字应当是记录数据各个文件在'压缩文件数据区'的位置（利用文件指针能够继续读取的特性）。

  3. 在文件头记录文件的目录结构、偏移量：其中目录结构使用前缀索引（将会在前缀记录所有文件目录，数据区存储包含文件后缀的的文件名）、偏移量单独在文件后存储（为每个节点预留三十二字节）

  4. 对于动态获取的偏移量的实时存储（接到结束信号后将文件原大小及其压缩偏移量从缓冲区写入文件）。
    目前的方案是将文件头和数据块的数据分别写入两个文件，最后进行组合（使用魔数分割）
    临时文件名的方案：
    [压缩输出名称][功能][时间戳][随机数][进程ID].tmp 尽量避免文件名冲突
    关于合并操作：使用流式合并


  5. 对于文件结构的存储方案：标记深度的分类前缀表
...
- **ThreadManager**
  ...
  * 多线程流水线：具体来说 使用一一对应的生产者-消费者构建数个流水线，并且运用双缓冲技术赋予上下游异步读写的能力，实现提速。 可以多线程地使用多条流水线。（后续可以使用环形缓冲区进一步优化上下游之间的数据流）
