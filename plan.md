...
**BufferPool**
- 通信缓存池：压缩与加密端之间的通信。依赖于文件头包含的顺序标签（保证按序组合）以及终止信号（抛出）
- 打包缓存池：打包压缩单独输出、压缩加密输出（利用文件头区分。可能分别设计两类接口）
- 解包缓存池：直接由UnitManager控制目录的收发，打包时将目录存放到文件头，配合文件分割标志（是一个经过读取缓存池转换后，赋给数据块对象的变量的bool类型变量值）来用于此处的目录还原。
- 读取缓存池：读取目录并传输至打包缓存池、检查文件的终止符（检测到后截断文件读取）。
- 数据块类：
'''
