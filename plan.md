...
<<<<<<< HEAD
  1. 通信缓存池：压缩与加密端之间的通信。依赖于文件头包含的顺序标签（保证按序组合）以及终止信号（抛出）
=======
  1. 通信缓存池：压缩与加密端之间的通信。
>>>>>>> koharu
...
<<<<<<< HEAD
  4. 读取缓存池：读取目录并传输至打包缓存池、检查文件的终止符（检测到后截断文件读取）。

- **另外：**
  * 文件信息处理器：
  1. 将文件的整个文件头读取到程序缓存中，并进行相关检查。

  * 关于数据块类：
  ```
    vector<uint8_t> data    // 数据（二进制）
=======
  4. 读取缓存池：读取目录并传输至打包缓存池。读取文件后：为压缩模块发送数据块、为解压模块发送数据块。
...
- **关于断点的记录方式和相关规则：**
<<<<<<< HEAD
  * 维护：根据数据块对象的data.size()方法动态维护文件大小，直至遇到终止信号，把文件名及其大小写入一个临时文件后准备下一次的记录。
=======
  * 维护：在文件头中存储数据文件的文件名初始大小，解压时逐步递减，直至字节数不足。另外，需要在压缩时在一个临时文件内动态维护已压缩文件的大小，可以用于多文件下的heffman与aes的部分功能实现
>>>>>>> koharu

- **断点细分：**
  1. 记录文件头和数据块的断点：使用魔数区分（文件头做不到记录文件在整个压缩文件的绝对位置，所以需要记录从数据区开始的文件的大小。且文件头数据量小）

  2. 记录数据块之间的断点：具体来说，这些数字应当是记录数据各个文件在'压缩文件数据区'的位置（利用文件指针能够继续读取的特性）。
<<<<<<< HEAD
=======
