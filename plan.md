...
- **关于文件头：**
  ...
  - II:文件头包含的信息是规定好的，不会出现预料外的数据，故用更短的休止符方便检测是更好的选择（0xff等）
  ...
  5. 实现细则：
      - 1. 存储协议：`|16bit文件名大小偏移量|文件名|文件标|----|`
          - 文件标为0（目录）：写入 `|16bit第一级文件数量|`
          - 文件标为1（文件）：写入 `|32bit原文件大小|32bit压缩大小|`（压缩大小占位）
      - 2. 运行流程：
          - 0. **文件头**：首尾需写入魔数（Magic Number）
          - 1. **目录处理**：
              - 读取后按存储协议写入
              - 缓冲区缓存当前目录的文件数量
          - 2. **文件处理**：直接按存储协议写入
          - 3. **搜索方案（BFS）**：
              - **I**. 进入文件夹，记录：
                  - 第二级文件数量
                  - 目录完整路径（存缓冲区）
              - **II**. 返回第一级目录，重复 **I** 直至第一级搜索完成
              - **III**. 按缓冲区路径打开下一级目录：
                  - 按顺序读写文件数据
                  - 直至该层级文件数量耗尽
              - **IV**. 循环 **I~III** 直到全部读取完成
              > 此方案解决解压时的文件顺序读取问题
      - 3. 偏移量操作：
          - 使用 `<fstream>` 的 `tellg()`/`tellp()` 和 `seekg()`/`seekp()`
          - **关键步骤**：文件读取完成后，在文件头的偏移占位处回写实际大小
