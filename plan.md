...
- **关于文件头：**
  ...
  5. 实现细则：
    ...
    - 1. 存储协议：|文件标|8bits文件名大小偏移量|文件名|----|
      - 文件标为0：为目录。需要写入：|32bits第一级文件数量|
      - 文件标为1：为文件。需要写入：|64bits原文件大小|64bit压缩大小|。其中压缩大小暂时作为占位区。
...
```
      文件头:
      ├─------Magic------
      ├─[文件头偏移量] |64bits|---------------(便于解压程序直接定位到数据区)
      ├─[名称]   |偏移量|文件名.sy|
      ├─[Flags]  |偏移量|数个字符型flag|------(压缩加密方案、版本号)
      ├─------Magic------
      ├─[目录信息]
      │ |目录||目录||目录|
      │ |目录||目录||目录|
      ├─------Magic------____________________(此处以上均为文件头)
      ├─[数据区]
      │ |数据块||数据块||数据块|
      │ |数据块||数据块||数据块|
      ├─------Magic------
