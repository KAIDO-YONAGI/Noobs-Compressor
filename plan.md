...
- 3.解包缓存池：直接由UnitManager控制目录的收发，打包时将目录存放到文件头，配合文件分割标志（是一个经过读取缓存池转换后，赋给数据块对象的变量的bool类型变量值）来用于此处的目录还原。

- 4.读取缓存池：读取目录并传输至打包缓存池、检查文件的终止符（检测到后截断文件读取）。
- 
- 另外：
- 文件信息处理器：
- 将文件的整个文件头读取到程序缓存中，并进行相关检查。
- 
- 关于数据块类：
'''
...
- 断点细分：
- I：记录文件头和数据块的断点：使用魔数区分（文件头做不到记录文件在整个压缩文件的绝对位置，所以需要记录从数据区开始的文件的大小。且文件头数据量小）

- II：记录数据块之间的断点：具体来说，这些数字应当是记录数据各个文件在'压缩文件数据区'的位置（利用文件指针能够继续读取的特性）。
**GUI**  
设计GUI：
- I：多线程模式选择(后续自动检测)
- II：加密、压缩算法切换
