...
writeFileStandard() - 文件标准写入 (src/BinaryIO_Writer.cpp:95)
  写入内容 (按序):
    ...
    2. fileNameSize (sizeof(FileNameSize)) - 文件名长度
    3. fileName (fileNameSize字节) - 文件名
    4. originSize (sizeof(FileSize)) - 原始文件大小 (未压缩)
    5. compressedSize占位 (sizeof(FileSize)) - 预留为0, 后续压缩完回填
  更新: tempOffset += FLAG_SIZE + sizeof(FileNameSize) + fileNameSize + 2*sizeof(FileSize)

writeDirectoryStandard() - 目录标准写入 (src/BinaryIO_Writer.cpp:83)
  写入内容 (按序):
    1. DIRECTORY_FLAG (1字节) - 标记这是一个目录
    2. directoryNameSize (sizeof(FileNameSize)) - 目录名长度
    3. directoryName (directoryNameSize字节) - 目录名
    4. childCount (sizeof(FileCount)) - 该目录下的直接子元素数量
  目录队列: 同时将此目录及其子元素数量入队 {directoryDetails, childCount}
  更新: tempOffset += FLAG_SIZE + sizeof(FileNameSize) + directoryNameSize + sizeof(FileCount)

writeSymbolLinkStandard() - 符号链接写入 (src/BinaryIO_Writer.cpp:128)
  写入内容 (按序):
    1. SYMBOL_LINK_FLAG (1字符) - 标记这是符号链接
    2. linkNameSize (sizeof(FileNameSize)) - 链接名称长度
    3. targetPathSize (sizeof(FileNameSize)) - 目标路径长度
...
writeLogicalRoot() - 逻辑根写入 (src/BinaryIO_Writer.cpp:143)
  写入内容 (按序):
    ...
    2. rootNameSize (sizeof(FileNameSize)) - 根名称长度
    3. rootName (rootNameSize字节) - 根名称
    4. childCount (sizeof(FileCount)) - 根节点下的直接子元素数量
...
writeBlankSeparatedStandard() - 空分割标准预留 (src/BinaryIO_Writer.cpp:115)
  写入内容:
    ...
    2. blockSize占位 (sizeof(DirectoryOffsetSize)) - 预留为0
    3. ivNum占位 (sizeof(IvSize)) - 预留为0, 用于解密时的IV
...
HeaderWriter_v0::writeHeader() - 文件头写入 (src/HeaderWriter.cpp:3)
  写入位置: 紧接第一个魔数之后
  写入内容 (按序):
    1. STRATEGY (sizeof(CompressStrategy)) - 压缩策略(1=Huffman)
    2. VERSION (sizeof(CompressorVersion)) - 版本号
    3. headerOffset占位 (sizeof(HeaderOffsetSize)) - 预留为0
    4. directoryOffset占位 (sizeof(DirectoryOffsetSize)) - 预留为0
...
headerLoaderIterator() - 主读取函数 (src/BinaryIO_Loader.cpp:3)
  ...
  执行流程:
    ...
    2. 若offset == sizeof(SizeOfMagicNum)(4字节):
       - 读取末尾魔数进行验证
       - 调用 allLoopDone() 标记完成
       - 返回
    3. 当offset > 0:
...
fileParser() - 文件解析 (src/Directory_FileParser.cpp:28)
  提取内容:
    ...
    2. 调用 numsParser<FileSize>() 读取原始文件大小
    3. 区分压缩/解压模式:
       压缩模式(parserMode==1):
...
numsParser<T>() (src/Directory_FileParser.cpp:50)
  ...
  支持类型: FileSize, FileCount, DirectoryOffsetSize等
...
缓冲和偏移常量:
  ...
  SEPARATED_STANDARD_SIZE = FLAG_SIZE + sizeof(DirectoryOffsetSize) + sizeof(IvSize)
