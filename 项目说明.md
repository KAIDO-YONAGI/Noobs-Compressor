...
EntryProcessor (目录文件处理器)
文件位置: CompressorFileSystem/DataCommunication/include/DiEntryocessor.h
         CompressorFileSystem/DataCommunication/src/EntryProcessor.cpp

核心职责: 递归扫描源目录树，使用BFS(广度优先搜索)遍历，生成目录结构的二进制编码

entryProcessor() - 主入口函数 (src/DirEntrycessor.cpp:4)
  输入: filePathToScan (用户选择的路径列表)
        fullOutPath (输出压缩文件路径)
        logicalRoot (逻辑根名称)
  功能流程:
    1. 初始化偏移量: offset = HEADER_SIZE (11字节), tempOffset = 0
    2. 调用 BinaryIO_WWriter:writeBlankSeparatedStandard() 预留分割标准位置
    3. 调用 BinaryIO_WWriter:writeLogicalRoot() 写入逻辑根节点
       - 记录根节点标志 (LOGICAL_ROOT_FLAG)
       - 记录根节点名称大小和内容
       - 记录根节点下的子元素总数
    4. 调用 BinaryIO_WWriter:writeRoot() 处理filePathToScan中的各路径
       - 对于每个路径: 若是文件则记录; 若是目录则入队
    5. 进入BFS循环, 调用 scaflowScanner持续处理目录队列

flowScanner() - BFS遍历函数 (src/DiEntryocessor.cpp:43)
  功能: 维护队列式循环, 逐个处理directoryQueue中的目录
  循环过程:
    当 ddirectory_FileQueue非空:
      取出队首目录
      调用 BinaryIO_WWriter:binaryIStandardWriter) 扫描该目录内容
      返回后队首目录出队
  结果: 所有子目录被递归入队并处理, 直到队列为空


BinaryStandardWriter (二进制IO写入器)
文件位置: CompressorFileSystem/DataCommunication/include/BinaryIOStandardWriter
         CompressorFileSystem/DataCommunication/src/BinaryStandardWriter.cpp

核心职责: 目录/文件的单层扫描与二进制编码, 字节级写入与偏移回填

binaryStandardWriter() - 单层扫描函数 (src/BinaryIO_StandardWriterp:2)
...
writeStorageStandard() - 标准分发函数 (src/BinaryIO_StandardWriterp:51)
...
writeFileStandard() - 文件标准写入 (src/BinaryIO_StandardWriterp:95)
  写入内容 (按序):
    1. FILE_FLAG (1字节) - 标记这是一个文件
    2. fileNameSize (sizeof(Y_flib::FileNameSize)) - 文件名长度
    3. fileName (fileNameSize字节) - 文件名
    4. originSize (sizeof(Y_flib::FileSize)) - 原始文件大小 (未压缩)
    5. compressedSize占位 (sizeof(FfY_flib:FileSize) - 预留为0, 后续压缩完回填
  更新: tempOffset += FLAG_SIZE + sizeof(FfY_flib:FileNameSize + fileNameSize + 2*sizeof(FfY_flib:FileSize

writeDirectoryStandard() - 目录标准写入 (src/BinaryIO_StandardWriterp:83)
  写入内容 (按序):
    1. DIRECTORY_FLAG (1字节) - 标记这是一个目录
    2. directoryNameSize (sizeof(Y_flib::FileNameSize)) - 目录名长度
    3. directoryName (directoryNameSize字节) - 目录名
    4. childCount (sizeof(Y_flib::FileCount)) - 该目录下的直接子元素数量
  目录队列: 同时将此目录及其子元素数量入队 {directoryDetails, childCount}
  更新: tempOffset += FLAG_SIZE + sizeof(FfY_flib:FileNameSize + directoryNameSize + sizeof(FfY_flib:FileCount

writeSymbolLinkStandard() - 符号链接写入 (src/BinaryIO_StandardWriterp:128)
  写入内容 (按序):
    1. SYMBOL_LINK_FLAG (1字符) - 标记这是符号链接
    2. linkNameSize (sizeof(Y_flib::FileNameSize)) - 链接名称长度
    3. targetPathSize (sizeof(Y_flib::FileNameSize)) - 目标路径长度
    4. linkName (linkNameSize字节) - 链接名称
    5. targetPath (targetPathSize字节) - 目标路径
  特性: 符号链接不递归展开, 仅记录链接关系

writeLogicalRoot() - 逻辑根写入 (src/BinaryIO_StandardWriterp:143)
  写入内容 (按序):
    1. LOGICAL_ROOT_FLAG (1字节) - 标记逻辑根
    2. rootNameSize (sizeof(Y_flib::FileNameSize)) - 根名称长度
    3. rootName (rootNameSize字节) - 根名称
    4. childCount (sizeof(Y_flib::FileCount)) - 根节点下的直接子元素数量
  特性: 逻辑根是虚拟节点, 用于处理多路径压缩情况

writeSeparatedStandard() - 块标准回填 (src/BinaryIO_StandardWriterp:108)
  功能: 回填前面预留的分割标准中的块大小字段
  操作:
    1. 计算回填位置: offset + FLAG_SIZE
    2. 使用 locator.olocateFromBegin) 移动写指针到回填位置
    3. 写入 tempOffset (当前块的实际大小)
    4. 移动写指针到文件末尾继续写入

writeBlankSeparatedStandard() - 空分割标准预留 (src/BinaryIO_WStandardWriter:115)
  写入内容:
    1. SEPARATED_FLAG (1字节) - 分割标志
    2. blockSize占位 (sizeof(DfY_flib:DirectoryOffsetSize) - 预留为0
    3. ivNum占位 (sizeof(IfY_flib:IvSize) - 预留为0, 用于解密时的IV
  目的: 为下一块预留结构头部, 供后续回填使用

writeBlankSeparatedStandardForEncryption() - 加密模式分割标准 (src/BinaryIO_WStandardWriter:122)
  写入内容:
    1. SEPARATED_FLAG (1字节) - 分割标志
    2. blockSize占位 (sizeof(DfY_flib:DirectoryOffsetSize) - 预留为0
...
HeaderWriter_v0::writeHeader() - 文件头写入 (src/HeaderWriter.cpp:3)
  写入位置: 紧接第一个魔数之后
  写入内容 (按序):
    1. STRATEGY (sizeof(Y_flib::CompressStrategy)) - 压缩策略(1=Huffman)
    2. VERSION (sizeof(Y_flib::CompressorVersion)) - 版本号
    3. headerOffset占位 (sizeof(HfY_flib:HeaderOffsetSize) - 预留为0
    4. directoryOffset占位 (sizeof(DfY_flib:DirectoryOffsetSize) - 预留为0
  总长度: 11字节 (4+1+2+4, 包括第一个魔数)
  回填: headerOffset被回填为11 (表示整个文件头大小)

HeaderWriter_v0::writeDirectory() - 目录委托 (src/HeaderWriter.cpp:27)
  职责: 作为连接层, 将目录写入任务委托给 DirectoryEntry
  步骤:
    1. 创建 DEntryrocessor 对象
    2. 调用其 entryProcessor() 方法执行BFS扫描和二进制编码
...
BinaryStandardLoader (二进制IO加载器)
文件位置: CompressorFileSystem/DataCommunication/include/BinaryIOStandardLoader
         CompressorFileSystem/DataCommunication/src/BinaryStandardLoader.cpp

核心职责: 从.sy文件读取目录块, 解密并解析目录结构, 提供文件队列供解压流程使用

headerLoaderIterator() - 主读取函数 (src/BinaryIO_StandardLoaderp:3)
  输入: Aes对象 (用于解密目录块)
  执行流程:
    1. 首次调用时(tellg()==0):
    ...
    2. 若offset == sizeof(Y_flib::SizeOfMagicNum)(4字节):
       - 读取末尾魔数进行验证
       - 调用 allLoopDone() 标记完成
       - 返回
    3. 当offset > 0:
       - 循环调用 loadBySeSeparatedag() 逐块读取和解析目录数据
       - 每次调用减少offset

loadBySeparatedFlag() - 块读取与解析 (src/BinaryIO_StandardLoaderp:62)
  每个数据块的处理:
    ...
    9. 调用 DEntryarser::parser() 解析buffer中的目录条目
       - parser()负责逐条目解析并入队(fileQueue/direcdirectory_FileQueue
    10. 若 tempOffset == 0: offset -= readSize (到达末尾)

【重要队列】
fileQueue - 文件队列
  存储: {DEntryetails, compressedSize}对
  用途: 解压时逐个取出文件对象, 从.sy文件提取压缩数据进行解压

directory_FileQueue - 目录队列
  存储: {DEntryetails, childCount}对
...
EntryParser (目录文件解析器)
文件位置: CompressorFileSystem/DataCommunication/include/DiEntryrser.h
         CompressorFileSystem/DataCommunication/src/EntryParser.cpp

核心职责: 从二进制buffer中解析目录结构, 构造Directory_FileDetails对象并入队

parser() - 主解析分发 (src/DirEntryser.cpp:138)
  输入: bufferPtr (当前解析位置, 引用传递, 自动更新)
       countOfChildDirectory (当前处理中目录的子元素数, 引用传递)
  处理:
    ...
    3. 递减 ccountOfChildDirectory

fileParser() - 文件解析 (src/DiEntryrser.cpp:28)
  提取内容:
    1. 调用 ffileDetailsParser) 读取文件名长度和内容
    2. 调用 numsParser<FfY_flib:FileSize() 读取原始文件大小
    3. 区分压缩/解压模式:
       压缩模式(parserMode==1):
         - offsetToFill = 根据当前全局偏移计算回填位置
         - 跳过预留的compressedSize字段(不实际读值)
       解压模式(parserMode==2):
         - offsetToFill = 读取实际的压缩大小
    4. 调用 pathConnector() 拼接完整路径
    5. 构造 DiEntrytails 对象
    6. 将 {fileDetails, coffsetToFill 入队fileQueue

directoryParser() - 目录解析 (src/DiEntryrser.cpp:60)
  提取内容:
    1. 读取目录名长度和内容
    2. 读取子元素数量 count
    3. 拼接完整路径
    4. 构造 DiEntrytails 对象
    5. 将 {directoryDetails, count} 入队directoryQueue

rootParser() - 逻辑根解析 (src/DirEntryser.cpp:77)
  在压缩模式(parserMode==1):
...
【解析工具函数】
fileDetailsParser() (src/EntryParser.cpp:22)
  从buffer读取: [大小字段] [字符串内容]
  安全措施: 使用checkBounds()验证边界, 使用assign()避免未初始化问题

numsParser<T>() (src/EntryParser.cpp:50)
  通用数值解析: 从buffer按字节读取T类型的数据
  支持类型: FiflY_flibFileSizeFiflY_flibFileCountDirectoryOffsetSize等

checkBounds() (src/EntryParser.cpp:3)
...
exportCompressedData() - 压缩数据导出 (src/DataExporter.cpp:24)
  用于: 压缩循环中每次数据块导出
  流程:
    1. 移动写指针到文件末尾
    2. 调用 writeBlankSeparatedStandardForEncryption() 写分割标准头(预留大小)
    3. 从outFile.write() 写入加密后的压缩数据块
    4. 累计到processedFfY_flib:FileSize
    5. 调用 thisBlockIsDone() 回填块大小

exportDecompressedData() - 解压数据导出 (src/DataExporter.cpp:39)
  用于: 解压循环中每次数据块导出
  流程:
    1. 移动写指针到文件末尾
...
thisFileIsDone() - 文件完成回填 (src/DataExporter.cpp:14)
  ...
  操作:
    ...
    2. 写入processedFfY_flib:FileSize该文件压缩后的总大小)
...
压缩中的加密:
  1. CompressionLoop::compressionLoop() 中:
     a. 对Huffman树进行序列化得到 huffTree (DataflibY_flibtaBlock
     b. 调用 aes.doAes(1, huffTree, hhuffTreeOutPut 加密树
     c. 调用 dataExporter.exportDataTCompressionhhuffTreeOutPut 写入
  2. 对压缩数据进行加密:
     a. Huffman编码得到 compressedData
     b. 调用 aes.doAes(1, compressedData, encryptedBlock) 加密数据
     c. 调用 dataExporter.exportDataTCompressionencryptedBlock) 写入

解压中的解密:
  1. DecompressionLoop::decompressionLoop() 中:
     a. BinaryStandardLoader 读取加密的树数据块
...
压缩主循环:
  CompressionLoop::compressionLoop() {
    初始化 BinaryIOStandardLoader读目录块)
    初始化 Heffman
    初始化 DataExporter
    ...
    while (fileQueue 非空) {
      ...
      for (每个数据块) {
        ...
        dataExporter.exportCompressedData(encryptedTree) - 写入树

        huffmanZip.encode(dataBlock, compressedData) - 压缩数据
        aes.doAes(1, compressedData, encryptedBlock) - 加密数据
        dataExporter.exportCompressedData(encryptedBlock) - 写入数据
      }
...
解压主循环:
  DecompressionLoop::decompressionLoop() {
    ...
    while (directory_FileQueue 非空 或 fileQueue 非空) {
      ...
      while (fileQueue 非空) {
        ...
        while (该文件数据未读完) {
          ...
          dataExporter.exportDecompressedData(decompressedData) - 写入文件
        }
      }
...
解压:
  .sy文件
    |
    v
  BinaryStandardLoader::loadBySeparatedFlag()
    |
    v
  加密后的树 + 加密后的数据
    |
...
缓冲和偏移常量:
  ...
  SEPARATED_STANDARD_SIZE = FLAG_SIZE + sizeof(Y_flib::DirectoryOffsetSize) + sizeof(Y_flib::IvSize)
  HEADER_SIZE = 11 (字节) - 文件头大小(包括第一个魔数)
  MAGIC_NUM = 0xDEADBEEF (4字节) - 文件标记

类型定义:
  Y_flib::DataBlock = std::vector<uint8_t> - 数据块向量
  EntryQueue = std::queue<{EntryDetails, uint64_t}> - 目录或文件队列
