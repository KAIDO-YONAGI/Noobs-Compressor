...
void HeaderWriter_v0::writeHeader(std::ofstream &outFile, fs::path &fullOutPath)
{
    ...
    numWriter.BIO.write_binary_le(outFile, strategySize);
    numWriter.BIO.write_binary_le(outFile, versionSize);
    numWriter.BIO.write_binary_le(outFile, headerOffsetSize);
    numWriter.BIO.write_binary_le(outFile, directoryOffsetSize);
    ...
    numWriter.BIO.write_binary_le(outFile, headerOffset);
...
void HeaderWriter_v0::writeDirectory(std::ofstream &outFile, const std::vector<std::string> &filePathToScan, const fs::path &fullOutPath, const std::string &logicalRoot)
{
    ...
    numWriter.BIO.write_binary_le(outFile, directoryOffset);
    outFile.seekp(0, std::ios::end);
}
